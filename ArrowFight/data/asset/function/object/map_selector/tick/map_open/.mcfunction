...
# 生成ボタンを召喚
    # ジャングル
        ...
        execute positioned ^1 ^1 ^ rotated 180 0 run function api:object/summon.m {ID:button_map_generate}
    # タウン
        ...
        execute store result storage api: Argument.FieldOverride.Scale[1] float 1 run scoreboard players get @s Temporary
        execute positioned ^2 ^1 ^ rotated 180 0 run function api:object/summon.m {ID:button_map_generate}
    # アイス
        data modify storage api: Argument.FieldOverride set value {MapName:"アイス",MapIcon:"minecraft:packed_ice",MapID:"ice"}
        execute store result storage api: Argument.FieldOverride.Scale[1] float 1 run scoreboard players get @s Temporary
        execute positioned ^3 ^1 ^ rotated 180 0 run function api:object/summon.m {ID:button_map_generate}
    # サンド
